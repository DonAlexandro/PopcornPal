...
export function setupBot(token: string) {
    ...
    bot.onText(/\/start/, (msg) => {
        bot.sendMessage(msg.chat.id, 'Привіт! Натисни кнопку "Знайти кіно", щоб отримати випадковий фільм.', keyboard);
    });
    ...
    bot.on('message', async (msg) => {
        ...
        if (userState && userState.step === 'AWAITING_RATING') {
            const rating = Number(text.trim());
            
            if (isNaN(rating) || rating < 1 || rating > 5) {
                await bot.sendMessage(chatId, 'Оцінка не може бути нижчою за 1 чи більшою за 5');
            ...
            try {
                ...
                await bot.sendMessage(chatId, 'Щось пішло не так при оновленні сторінки в Notion');
                userStates.delete(chatId);
            }
            return;
        }
        ...
        try {
            ...
            await bot.sendMessage(chatId, `Знайдено фільм: "${notionMovie.title}". Шукаю деталі на TMDB...`);

            const moviesDetails = await fetchMovieDetails(notionMovie.title);

            if (!moviesDetails || moviesDetails.length === 0) {
                await bot.sendMessage(chatId, `Не вдалося знайти деталі для фільму "${notionMovie.title}" на TMDB. 😢`);
                return;
            }

            userStates.set(chatId, { step: 'VIEWING_OPTIONS', pageId: notionMovie.id, options: moviesDetails.map(m => m.genres) });
            ...
            for (let i = 0; i < moviesDetails.length; i++) {
                ...
                const inlineKeyboard = {
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: '✅ Переглянув', callback_data: `watched_${notionMovie.id}_${i}` }]
                        ]
                    }
                };

                if (movieDetails.posterUrl) {
                    await bot.sendPhoto(chatId, movieDetails.posterUrl, { parse_mode: 'HTML', caption, ...inlineKeyboard });
                } else {
    ...
    bot.on('callback_query', async (query) => {
        ...
        if (query.data && query.data.startsWith('watched_')) {
            ...
            if (!userState || userState.pageId !== pageId || !userState.options) {
                await bot.answerCallbackQuery(query.id, { text: 'Дані застаріли. Знайдіть фільм знову.', show_alert: true });
                return;
            }
