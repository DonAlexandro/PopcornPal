...
export async function getRandomMovieFromNotion(): Promise<{ id: string; title: string }> {
    ...
    try {
        ...
        const titleProperty = Object.values(randomPage.properties).find((prop: any) => prop.type === 'title') as any;
...
export async function updateMovieInNotion(pageId: string, rating: number, genres: string[]) {
    const notion = new Client({ auth: process.env.NOTION_API_KEY });
