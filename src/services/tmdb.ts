...
export async function fetchMovieDetails(title: string): Promise<MovieDetails[]> {
    ...
    const trySearch = async (language: string): Promise<number[] | null> => {
        ...
        return data.results && data.results.length > 0 ? data.results.map((r: any) => r.id) : null;
    };

    // Try finding the movie with different language fallbacks
    let movieIds = await trySearch('uk-UA');
    if (!movieIds) movieIds = await trySearch('ru-RU');
    if (!movieIds) movieIds = await trySearch('en-US');
    ...
    const fetchSingleMovie = async (movieId: number): Promise<MovieDetails | null> => {
        // Fetch full details
